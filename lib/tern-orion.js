define('orion/tern/ternServer', function() {
  ...
  TernServer.prototype = {
    ...
    addContentAssistProvider: function(editor, options) {
      var contentAssist = editor.getContentAssist ? editor.getContentAssist() : editor._contentAssist;
      var ternProvider = this.getContentAssistProvider(editor, options);
      var version = 4;
      if (contentAssist.providers) version = 4; else if (contentAssist.getProviders) version = 6; else version = 5; 
      
      function addTernProvider() {
        if (options && options.charTriggers) {
          ...
          case 5:
            ...
            if (infos) {
              for(var i=0; i<infos.length; i++) {
                  providers.push(infos[i]);
              }                               
            }
            providers.push(ternProvider);
            contentAssist.setProviderInfoArray(providers);            
            break;
          case 6:
      ...
      contentAssist.addEventListener("Activating", addTernProvider);
    },
    
    getContentAssistProvider: function(editor, options) {
      if (!options) options = {};
      if (!options.id) options.id = 'tern';
      options.provider = new TernContentAssistProvider(this, editor);
      return options;
    },
